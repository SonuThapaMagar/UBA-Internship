  ...
  return {
    AppDataSource: {
      getRepository: vi.fn().mockImplementation((entity) => {
        ...
        if (entity === Internship) return mockInternRepo;
        return {};
      }),
    },
  };
...
describe('UserService', () => {
  ...
  describe('deleteUser', () => {
    it('should delete a user and return it', async () => {
      ...
      mockUserRepo.findOne.mockResolvedValue(mockUser);
      mockUserRepo.remove.mockResolvedValue(mockUser);
      internRepoMock.delete.mockResolvedValue({ affected: 1 });
      const result = await userService.deleteUser('1');
      expect(result).toEqual(mockUser);
      expect(mockUserRepo.findOne).toHaveBeenCalledWith({ where: { id: '1' } });
      expect(mockUserRepo.remove).toHaveBeenCalledWith(mockUser);
      expect(internRepoMock.delete).toHaveBeenCalledWith({ user: { id: '1' } });
    });
  ...
  describe('login', () => {
    ...
    it('should throw error for non-existent user', async () => {
      ...
      await expect(userService.login('nonexistent@example.com', 'password123')).rejects.toThrow('Invalid credentials');
    });
  });
});